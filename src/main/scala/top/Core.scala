...
class Core(implicit val p: MarCoreConfig) extends MarCoreModule {
  ...
  class MarCoreIO extends Bundle {
    ...
    val gpr =
      if (Settings.get("EnableDifftest")) Some(new RegsDiffIO(num = 32))
      else None
    val csr =
      if (Settings.get("EnableDifftest")) Some(new RegsDiffIO(num = 4))
      else None
    val difftest_commit =
...
//
    io.dmem <> Cache(
      ...
      flush = "b00".U,
      empty = dtlb,
      swapPolicy = Settings.getString("DCacheSwapPolicy"),
      enable = HasDCache
    )(
      CacheConfig(
        ro = false,
    ...
    io.imem <> Cache(
      ...
      empty = itlb,
      swapPolicy = Settings.getString("ICacheSwapPolicy"),
      enable = HasICache
    )(
      CacheConfig(
        ro = true,
    ...
    Info(
      "flush = %b, 0: frontend:(%d,%d), backend:(%d,%d); " + "1: frontend:(%d,%d), backend:(%d,%d)\n",
      frontend.io.flushVec.asUInt,
      frontend.io.out(0).valid,
      frontend.io.out(0).ready,
      backend.io.in(0).valid,
      backend.io.in(0).ready,
      frontend.io.out(1).valid,
      frontend.io.out(1).ready,
      backend.io.in(1).valid,
      backend.io.in(1).ready
    )
    ...
    if (Settings.get("TraceLoadStore")) {
      ...
      Info(
        "[Inst] Req(%d,%d)\t   addr 0x%x len %d size %d user %x id %d; Resp(%d,%d) data 0x%x last %d user %x id %d\n",
        iReq.valid,
        iReq.ready,
        iReq.bits.addr,
        iReq.bits.len,
        iReq.bits.size,
        iReq.bits.user.getOrElse(0.U),
        iReq.bits.id.getOrElse(0.U),
        iResp.valid,
        iResp.ready,
        iResp.bits.data,
        iResp.bits.last,
        iResp.bits.user.getOrElse(0.U),
        iResp.bits.id.getOrElse(0.U)
      )
      Info(
        "[Data] Req(%d,%d) isW %d addr 0x%x len %d size %d data 0x%x strb %b last %d user %d id %d; Resp(%d,%d) isW %d data 0x%x last %d resp %d user %x id %d\n",
        dReq.valid,
        dReq.ready,
        dReq.bits.write,
        dReq.bits.addr,
        ...
        dReq.bits.strb,
        dReq.bits.last,
        dReq.bits.user.getOrElse(0.U),
        dReq.bits.id.getOrElse(0.U),
        dResp.valid,
        dResp.ready,
        dResp.bits.write,
        dResp.bits.data,
        dResp.bits.last,
        dResp.bits.user.getOrElse(0.U),
        dResp.bits.id.getOrElse(0.U)
      )
    }
    if (Settings.get("TraceMMIO")) {
      ...
      Info(
        "[MMIO] Req(%d,%d) isW %d addr 0x%x len %d size %d data 0x%x strb %b last %d user %d id %d; Resp(%d,%d) isW %d data 0x%x last %d resp %d user %x id %d\n",
        mReq.valid,
        mReq.ready,
        mReq.bits.write,
        mReq.bits.addr,
        ...
        mReq.bits.strb,
        mReq.bits.last,
        mReq.bits.user.getOrElse(0.U),
        mReq.bits.id.getOrElse(0.U),
        mResp.valid,
        mResp.ready,
        mResp.bits.write,
        mResp.bits.data,
        mResp.bits.last,
        mResp.bits.user.getOrElse(0.U),
        mResp.bits.id.getOrElse(0.U)
      )
    }
