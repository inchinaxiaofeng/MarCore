/*
 * 模块：SRT4DividerDataModule
 * 功能：基于 Radix-4 SRT 算法的整数除法器数据单元，支持带符号/无符号除法，
 *       商或余数输出，高低位选择，以及 32/64 位字模式（W 后缀）符号扩展。
 * 延迟：2~(5 + (len+3)/2) 周期完成一次除法。
 */
...
class SRT4DividerDataModule(len: Int) extends Module {
  val io = IO(new Bundle() {
    ...
    /** 写回阶段清除信号 */
    val kill_w = Input(Bool())

    /** 读/计算阶段清除信号 */
    val kill_r = Input(Bool())
  ...
  // ==== 常量与状态机声明 ====
  // consts
  val lzc_width = log2Up(len) // Leading zero 计数位宽
  val itn_len = 1 + len + 2 + 1 // 内部余数寄存器宽度
  require(lzc_width == 6, "Only support len=64 or len=32")

  // 状态：idle, 预处理 0, 预处理 1, 迭代, 后处理 0, 后处理 1, 完成
  val s_idle :: s_pre_0 :: s_pre_1 :: s_iter :: s_post_0 :: s_post_1 :: s_finish :: Nil =
    Enum(7)
  ...
  // reused other regs
  val aNormAbsReg = RegEnable(
    aNormAbs,
    startHandShake | state(s_pre_0) | state(s_post_0)
  ) // reg for normalized a & d and rem & rem+d
  val dNormAbsReg =
    RegEnable(dNormAbs, startHandShake | state(s_pre_0) | state(s_post_0))
  val quotIterReg =
  ...
  // == 记录归一化结果 ==
  val aNorm =
    (aNormAbsReg(len - 1, 0) << aLZC(lzc_width - 1, 0))(len - 1, 0) // 64, 65
  ...
  aNormAbs := Mux1H(
    Seq(
      ...
      state(s_post_0) -> rNext(
        len + 3,
        3
      ) // remainder 65, 64. highest is sign bit
    )
  )
  dNormAbs := Mux1H(
    Seq(
  ...
  // === 是否直接结束迭代条件 ===
  val lzcWireDiff = Cat(0.U(1.W), dLZC(lzc_width - 1, 0)) - Cat(
    0.U(1.W),
    aLZC(lzc_width - 1, 0)
  ) // 7, 0
  val lzcRegDiff = Cat(0.U(1.W), dLZCReg(lzc_width - 1, 0)) - Cat(
    0.U(1.W),
  ...
  val noIterReg = RegEnable(
    dIsOne & aNormAbsReg(len - 1),
    state(s_pre_0)
  ) // This means dividend has lzc 0 so iter is 17
  noIter := noIterReg
  val aTooSmallReg = RegEnable(
    aIsZero | lzcDiff(lzc_width),
    state(s_pre_0)
  ) // a is zero or a smaller than d
  ...
  val rSign = aSign
  val quotSignReg =
    RegEnable(quotSign, startHandShake | (state(s_pre_1) & dIsZero))
  val rSignReg = RegEnable(rSign, startHandShake)

  val rShift = lzcDiff(0) // odd lzc diff, for SRT4
  val rightShifted = Wire(UInt(len.W))
  val rSumInit = Mux(
    aTooSmallReg | aIsZero,
    Cat(
      0.U(1.W),
      rightShifted,
      0.U(3.W)
    ), // right shift the dividend (which is already l-shifted)
    Mux(
      noIterReg,
      0.U(itn_len.W), //
      Cat(
        ...
        Mux(
          rShift,
          Cat(0.U(1.W), aNormAbsReg(len - 1, 0)),
          Cat(aNormAbsReg(len - 1, 0), 0.U(1.W))
        )
  ...
  rightShifter.io.in := Mux(
    state(s_pre_1),
    aNormAbsReg(len - 1, 0),
    rPreShifted(len - 1, 0)
  )
  rightShifter.io.shiftNum := Mux(
    state(s_pre_1),
    aLZCReg,
  ...
  val rSumInitTrunc =
    Cat(0.U(1.W), rSumInit(itn_len - 4, itn_len - 4 - 4 + 1)) // 0.00___
  val mInitPos1 =
    MuxLookup(dNormAbsReg(len - 2, len - 2 - 3 + 1), "b00100".U(5.W))(
      Seq(
        0.U -> "b00100".U(5.W),
        1.U -> "b00100".U(5.W),
        2.U -> "b00100".U(5.W),
  ...
  val mInitPos2 =
    MuxLookup(dNormAbsReg(len - 2, len - 2 - 3 + 1), "b01100".U(5.W))(
      Seq(
        0.U -> "b01100".U(5.W),
        1.U -> "b01110".U(5.W),
        2.U -> "b01111".U(5.W),
  ...
  val qInit = Mux(
    initCmpPos2,
    UIntToOH(quot_pos_2, 5),
    Mux(initCmpPos1, UIntToOH(quot_pos_1, 5), UIntToOH(quot_0, 5))
  )
  val qPrev = Mux(state(s_pre_1), qInit, qIterEnd)
  val qPrevReg = RegEnable(qPrev, state(s_pre_1) | state(s_iter))
  val specialDivisorReg = RegEnable(
    dNormAbsReg(len - 2, len - 2 - 3 + 1) === 0.U,
    state(s_pre_1)
  ) // d=0.1000xxx
  // rCarry and rSum in Iteration
  val qXd = Mux1H(
    Seq(
      qPrevReg(quot_neg_2) -> Cat(
        dNormAbsReg(len - 1, 0),
        0.U(4.W)
      ), // 68, 67 1.xxxxx0000
      qPrevReg(quot_neg_1) -> Cat(
        0.U(1.W),
        dNormAbsReg(len - 1, 0),
        0.U(3.W)
      ), // 0.1xxxxx000
      qPrevReg(quot_0) -> 0.U(itn_len.W),
      qPrevReg(quot_pos_1) -> ~Cat(
        0.U(1.W),
        dNormAbsReg(len - 1, 0),
        0.U(3.W)
      ), // don't forget to plus 1 later
      qPrevReg(quot_pos_2) -> ~Cat(
        dNormAbsReg(len - 1, 0),
        0.U(4.W)
      ) // don't forget to plus 1 later
    )
  ...
  val rCarryIter = Cat(
    csa.io.out(1)(itn_len - 2, 0),
    qPrevReg(quot_pos_1) | qPrevReg(quot_pos_2)
  )
  val rSumReg = RegEnable(
    Mux(state(s_pre_1), rSumInit, rSumIter),
    state(s_pre_1) | state(s_iter)
  ) // 68, 67
  val rCarryReg = RegEnable(
    Mux(state(s_pre_1), rCarryInit, rCarryIter),
    state(s_pre_1) | state(s_iter)
  )
  ...
  qds.io.d := dNormAbsReg(
    len - 1,
    0
  ) // Maybe optimize here to lower power consumption?
  ...
  quotIter := Mux(
    ...
    Mux(state(s_iter), quotIterNext, Mux(quotSignReg, aInverter, quotIterReg))
  )
  quotM1Iter := Mux(
    state(s_pre_1),
    0.U(len.W),
  ...
  iterNum := Mux(
    state(s_pre_1),
    lzcDiff(lzc_width - 1, 1) +% lzcDiff(0),
    iterNumReg -% 1.U
  )
  ...
  val rPd = dNormAbsReg
  val rIsZero = ~(r.orR)
  val needCorr = (~dIsZero & ~noIterReg) & Mux(
    rSignReg,
    ~r(len) & ~rIsZero,
    r(len)
  ) // when we get pos rem for d<0 or neg rem for d>0
  rPreShifted := Mux(needCorr, rPd, r)
  val rFinal = RegEnable(
    rightShifted,
    state(s_post_1)
...
class RightShifter(len: Int, lzc_width: Int) extends Module {
  ...
  val s0 = Mux(
    shift(0),
    Cat(VecInit(Seq.fill(1)(msb)).asUInt, io.in(len - 1, 1)),
    io.in
  )
  val s1 =
    Mux(shift(1), Cat(VecInit(Seq.fill(2)(msb)).asUInt, s0(len - 1, 2)), s0)
  val s2 =
    Mux(shift(2), Cat(VecInit(Seq.fill(4)(msb)).asUInt, s1(len - 1, 4)), s1)
  val s3 =
    Mux(shift(3), Cat(VecInit(Seq.fill(8)(msb)).asUInt, s2(len - 1, 8)), s2)
  val s4 =
    Mux(shift(4), Cat(VecInit(Seq.fill(16)(msb)).asUInt, s3(len - 1, 16)), s3)
  val s5 = Wire(UInt(len.W))
  if (len == 64) {
    s5 := Mux(
      shift(5),
      Cat(VecInit(Seq.fill(32)(msb)).asUInt, s4(len - 1, 32)),
      s4
    )
  } else if (len == 32) {
    s5 := s4
    ...
    i =>
      {
        ...
        csa1.io.in(2) := MuxLookup(dForLookup, "b0000000".U)(
          mLookUpTable.minus_m(i)
        )
