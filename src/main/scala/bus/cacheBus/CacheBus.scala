...
class CacheBusReqBundle(
    val userBits: Int = 0,
    val idBits: Int = 0,
    val addrBits: Int = CacheBusParameters.addrBits,
    val dataBits: Int = CacheBusParameters.dataBits,
    val lenBits: Int = CacheBusParameters.lenBits,
    val sizeBits: Int = CacheBusParameters.sizeBits
) extends CacheBusBundle {
...
class CacheBusRespBundle(
    val userBits: Int = 0,
    val idBits: Int = 0,
    val addrBits: Int = CacheBusParameters.addrBits,
    val dataBits: Int = CacheBusParameters.dataBits,
    val respBits: Int = CacheBusParameters.respBits
) extends CacheBusBundle {
  val user = if (userBits > 0) Some(Output(UInt(userBits.W))) else None
  val id = if (idBits > 0) Some(Output(UInt(idBits.W))) else None
...
class CacheBus(
  ...
  def toAXI4(isFromCache: Boolean = false) =
    CacheBus2AXI4Converter(this, new AXI4, isFromCache)
