...
class CacheBus2AXI4Converter[OT <: AXI4Lite](outType: OT, isFromCache: Boolean)
    extends Module {
  ...
  val toAXI4Lite =
    !(io.in.req.valid && io.in.req.bits.len =/= 1.U) && (outType.getClass == classOf[
      AXI4Lite
    ]).B
  ...
  val (ar, aw, w, r, b) =
    (axi.ar.bits, axi.aw.bits, axi.w.bits, axi.r.bits, axi.b.bits)
