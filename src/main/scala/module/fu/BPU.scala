...
class BPU_embedded extends MarCoreModule {
	...
	val lphtsAddr = Cat(btbAddr.getIdx(io.in.pc.bits), bhr.read(btbAddr.getIdx(io.in.pc.bits)))
	...
	val lphtsAddrUpdt = Cat(btbAddr.getIdx(io.bpuUpdate.pc), bhr.read(btbAddr.getIdx(io.bpuUpdate.pc)))
	val gphtsAddrUpdt = Cat(btbAddr.getIdx(io.bpuUpdate.pc), ghr.read(0.U))
	val cphtAddrUpdt = btbAddr.getIdx(io.bpuUpdate.pc)

	val cnt = RegNext(Mux(cpht.read(cphtAddrUpdt)(1),
	...
	val lphtsAddrReq = Cat(btbAddr.getIdx(reqLatch.pc), bhr.read(btbAddr.getIdx(reqLatch.pc)))
	...
	when (reqLatch.valid && ALUCtrl.isBranch(reqLatch.fuCtrl)) {
		...
		val newCphtCnt = Mux(cphtGlobalReq,
			Mux(taken, Mux(cphtCnt === "b11".U, cphtCnt, cphtCnt + 1.U), cphtCnt - 1.U),
			Mux(taken, Mux(cphtCnt === "b00".U, cphtCnt, cphtCnt - 1.U), cphtCnt + 1.U)
		)
		...
		when (cphtGlobalReq) {
			...
			when (wen) {
				bhr.write(btbIdxReq, Cat(bhr.read(btbIdxReq)(log2Up(LNRpht)-2, 0), taken))
...
//	btbWrite.brIdx := Cat(io.bpuUpdate.pc(2,0)==="h6".U && !io.bpuUpdate.isRVC, io.bpuUpdate.pc(1), ~io.bpuUpdate.pc(1))
	btbWrite.brIdx := Cat(io.bpuUpdate.pc(2,0)==="h6".U, io.bpuUpdate.pc(1), ~io.bpuUpdate.pc(1))
	...
	val cnt = RegNext(Mux(cpht.read(cphtAddrUpdt)(phtLEN-1),
		gphts.read(gphtsAddrUpdt),
		lphts.read(lphtsAddrUpdt))
	)
	val reqLatch = RegNext(io.bpuUpdate)
	...
	val lphtsAddrReq = Cat(btbAddr.getIdx(reqLatch.pc), bhr.read(btbAddr.getIdx(reqLatch.pc)))
	val gphtsAddrReq = Cat(btbAddr.getIdx(reqLatch.pc), ghr.read(btbAddr.getIdx(reqLatch.pc)))
	val cphtAddrReq = btbAddr.getIdx(reqLatch.pc)
	...
	when (reqLatch.valid && ALUCtrl.isBranch(reqLatch.fuCtrl)) {
		...
		when (wen && cpht.read(cphtAddrReq)(phtLEN-1)) {
			ghr.write(btbIdxReq, Cat(ghr.read(btbIdxReq)(log2Up(GNRpht)-2, 0), taken))
			gphts.write(gphtsAddrReq, newCnt)
...
//	val rasWdata = Mux(rasResetState, 0.U, req.pc = Mux(io.bpuUpdate.isRVC, 2.U, 4.U))
	...
	val rasWen = rasResetState || (io.bpuUpdate.valid && (io.bpuUpdate.valid && (io.bpuUpdate.fuCtrl === ALUCtrl.call)))
	...
	io.out.valid := btbHit && Mux(btbRead._type === BTBtype.B, phtTaken, true.B && rasTarget =/= 0.U)
	io.out.rtype := 0.U
}
