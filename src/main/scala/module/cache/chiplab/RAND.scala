...
sealed class RANDStage1IO(implicit val cacheConfig: CacheConfig) extends CacheBundle {
	val req = new CacheBusReqBundle(userBits = userBits, idBits = idBits)
}

sealed class RANDCacheStage1(implicit val cacheConfig: CacheConfig) extends CacheModule with HasRANDConst {
...
sealed class RANDStage2IO(implicit val cacheConfig: CacheConfig) extends CacheBundle with HasRANDConst {
	val req = new CacheBusReqBundle(userBits = userBits, idBits = idBits)
...
sealed class RANDCacheStage2(implicit val cacheConfig: CacheConfig) extends CacheModule with HasRANDConst {
	implicit val moduleName: String = cacheName
...
sealed class RANDCacheStage3(implicit val cacheConfig: CacheConfig) extends CacheModule with HasRANDConst {
	implicit val moduleName: String = cacheName
...
//		statistic_cache.io.id := 0.U
	...
	io.mem.req.bits.apply(
		...
		len = (LineBeats-1).U)
	Debug("STATE %d STATE2 %d last %d\n", state, state2, writeBeatCnt.value === (LineBeats-1).U)
...
class RANDCache(implicit val cacheConfig: CacheConfig) extends CacheModule with HasCacheIO with HasRANDConst {
	implicit val moduleName: String = cacheName
