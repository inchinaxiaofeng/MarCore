...
sealed trait HasLRUConst extends HasCacheConst {
...
sealed class LRUStage1IO(implicit val cacheConfig: CacheConfig) extends CacheBundle {
	val req = new CacheBusReqBundle(userBits = userBits, idBits = idBits)
}

sealed class LRUCacheStage1(implicit val cacheConfig: CacheConfig) extends CacheModule with HasLRUConst {
	...
	class CacheStage1IO extends Bundle {
		...
		val out = Decoupled(new LRUStage1IO)
...
sealed class LRUStage2IO(implicit val cacheConfig: CacheConfig) extends CacheBundle with HasLRUConst {
	val req = new CacheBusReqBundle(userBits = userBits, idBits = idBits)
...
sealed class LRUCacheStage2(implicit val cacheConfig: CacheConfig) extends CacheModule with HasLRUConst {
	implicit val moduleName: String = cacheName
	class CacheStage2IO extends Bundle {
		val in = Flipped(Decoupled(new LRUStage1IO))
		val out = Decoupled(new LRUStage2IO)
	...
	val metaWay = Wire(Vec(Ways, chiselTypeOf(forwardMetaReg.data))) // Meta Data with way
	val pickForwardMeta = isForwardMetaReg || isForwardMeta // Use forward data
	val forwardMeta = Mux(isForwardMeta, io.metaWriteBus.req.bits, forwardMetaReg) // 不是当前的Forwarding，认为是旧的forwarding
	val forwardWaymask = forwardMeta.waymask.getOrElse("1".U).asBools
	// NOTE: 这里是用来检测从CacheArray中返回的值的
	Debug(io.in.valid, "[Read Resp(S2)] pickForward%d Forward v%d tag%x d%d data\n",
			pickForwardMeta, forwardMeta.data.valid, forwardMeta.data.tag, forwardMeta.data.dirty)
	forwardWaymask.zipWithIndex.map { case (w, i) =>
		metaWay(i) := Mux(pickForwardMeta && w, forwardMeta.data, io.metaReadResp(i))
		Debug(io.in.valid, "[Read Resp(S2)] ReadResp%d v%d tag%x d%d data%x\n",
			i.U, io.metaReadResp(i).valid, io.metaReadResp(i).tag, io.metaReadResp(i).dirty, io.dataReadResp(i).data)
	}

	val hitVec = VecInit(metaWay.map(m => m.valid && (m.tag === addr.tag) && io.in.valid)).asUInt // valid且tag相同为命中
//	Debug("Req Addr tag%x Vec1 v%d tag%x Vec1 v%d tag%x Vec2 v%d tag%x Vec3 v%d tag%x\n", addr.tag, metaWay(0).valid, metaWay(0).tag, metaWay(1).valid, metaWay(1).tag, metaWay(2).valid, metaWay(2).tag, metaWay(3).valid, metaWay(3).tag)
	...
	Debug(!io.out.bits.hit && !hasInvalidWay, "[Vectim] waymask %b\n", waymask.asUInt)
	assert(waymask.orR)
...
sealed class LRUCacheStage3(implicit val cacheConfig: CacheConfig) extends CacheModule with HasLRUConst {
	implicit val moduleName: String = cacheName
	class CacheStage3IO extends Bundle {
		val in = Flipped(Decoupled(new LRUStage2IO))
		val out = Decoupled(new CacheBusRespBundle(userBits = userBits, idBits = idBits))
	...
	val dataHitWriteBus = Wire(CacheDataArrayWriteBus()).apply(
		valid = hitWrite,
		setIdx = Cat(addr.index, Mux(req.write && req.len =/= 1.U, writeL2BeatCnt.value, addr.wordIndex)),
...
//	Debug("ReadBeatCnt %d WriteBeatCnt %d\n", readBeatCnt.value, writeBeatCnt.value)
	...
	io.mem.req.bits.apply(
		...
		write = state =/= s_memReadReq, // Write is default, cuz Write transfer request w chanel.
	...
	val inRdataRegDemand = RegEnable(Mux(mmio, io.mmio.resp.bits.data, io.mem.resp.bits.data), Mux(mmio, state === s_mmioResp, readingFirst))
	...
	switch (state) {
		is (s_idle) {
			...
			when (hitReadBurst && io.out.ready) {
				...
				Debug(meta.dirty, "DIRTY! Write Back!\n")
	...
	// dataRefill (cyan)
	val dataRefill = MaskData(io.mem.resp.bits.data, req.data, Mux(readingFirst, wordMask, 0.U(DataBits.W)))
	...
	val metaRefillWriteBus = Wire(CacheMetaArrayWriteBus()).apply(
		valid = (state === s_memReadResp) && io.mem.resp.fire && io.mem.resp.bits.last && !io.mem.resp.bits.write,
		setIdx = getMetaIdx(req.addr),
		data = Wire(new MetaBundle).apply(valid = true.B, tag = addr.tag, dirty = !ro.B && req.write),
		waymask = io.in.bits.waymask)
	...
	io.out.valid := io.in.valid && Mux((req.len =/= 1.U) && (cacheLevel == 2).B,
		Mux(req.write && (hit || !hit && state === s_wait_resp),
			true.B,
...
class LRUCache(implicit val cacheConfig: CacheConfig) extends CacheModule with HasCacheIO with HasLRUConst {
	implicit val moduleName: String = cacheName
	val io = IO(new CacheIO)

	val s1 = Module(new LRUCacheStage1)
	val s2 = Module(new LRUCacheStage2)
	val s3 = Module(new LRUCacheStage3)
	val metaArray = Module(new SRAMTemplateWithArbiter(nRead = 1, new MetaBundle, set = Sets, way = Ways, shouldReset = true, flushSet = false))
	val dataArray = Module(new SRAMTemplateWithArbiter(nRead = 2, new DataBundle, set = Sets * LineBeats, way = Ways, shouldReset = false, flushSet = false))
//	val lruMatrix = Module(Vec(Sets, new MatrixReg(Ways, Ways, 1)))
...
//	val lruWay = lruMatrix(s1.io.metaReadBus.req.setIdx).lru_row
	dataArray.io.r(0)	<> s1.io.dataReadBus
	dataArray.io.r(1)	<> s3.io.dataReadBus
//	lruMatrix(s3.io.metaWriteBus.req.setIdx).row_addr := s3.io.metaWriteBus.bits.waymask
//	lruMatrix(s3.io.metaWriteBus.req.setIdx).col_addr := s3.io.metaWriteBus.bits.waymask
	...
	Debug(s3.io.mmio.req.fire, "[MMIO REQ] addr%x len%d size%d data%x strb%b last%d write%d\n", s3.io.mmio.req.bits.addr, s3.io.mmio.req.bits.len, s3.io.mmio.req.bits.size, s3.io.mmio.req.bits.data, s3.io.mem.req.bits.strb, s3.io.mem.req.bits.last, s3.io.mem.req.bits.write)
	Debug(s3.io.mmio.resp.fire, "[MMIO RESP] data%x last%d write%d\n", s3.io.mmio.resp.bits.data, s3.io.mmio.resp.bits.last, s3.io.mmio.resp.bits.write)
