...
import core.frontend.unit.{IFU_embedded, IDU}

class FrontendIO(implicit val p: MarCoreConfig)
    extends Bundle
...
class Frontend_embedded(implicit val p: MarCoreConfig) extends MarCoreModule {
  ...
  Info("------------------------ Frontend ------------------------\n")
  Info(
    "flush = %b, {{{ [ifu]<%x--=%x%x>[idu]<%x%x== }}}\n",
    ifu.io.flushVec.asUInt,
    ifu.io.out.valid,
    idu.io.in(0).ready,
    idu.io.in(1).ready,
    idu.io.out(0).valid,
  ...
  Info(
    ...
    ifu.io.out.bits.pc,
    ifu.io.out.bits.instr
  )
  Info(
    idu.io.in(0).valid,
    "IDU1: pc = 0x%x, instr = 0x%x, pnpc = 0x%x\n",
    idu.io.in(0).bits.pc,
    idu.io.in(0).bits.instr,
    idu.io.in(0).bits.pnpc
  )
  Info(
    idu.io.in(1).valid,
    "IDU2: pc = 0x%x, instr = 0x%x, pnpc = 0x%x\n",
    idu.io.in(1).bits.pc,
    idu.io.in(1).bits.instr,
