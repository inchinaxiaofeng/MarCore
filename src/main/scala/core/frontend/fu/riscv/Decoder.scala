...
class RISCVDecoder(implicit val p: MarCoreConfig)
    extends MarCoreDeModule
    with HasRISCV_InstrType {
  ...
  val decodeList =
    ListLookup(
      instr,
      InstructionsRISCV.DecodeDefault,
      InstructionsRISCV.DecodeTable
    )
  val instrType :: fuType :: fuCtrl :: Nil =
    InstructionsRISCV.DecodeDefault.zip(decodeList).map { case (instr, dec) =>
      Mux(
