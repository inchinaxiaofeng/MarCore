...
class IFU_inorder
  ...
  val io = IO(new Bundle {
    val imem =
      new CacheBus(addrBits = VAddrBits, userBits = ICacheUserBundleWidth)
  ...
  when(pcUpdate || bpu.io.flush) {
    crosslineJumpLatch := Mux(
      bpu.io.flush,
      false.B,
      crosslineJump && !crosslineJumpLatch
    )
  }
  ...
  val npc = Mux(
    ...
    Mux(
      crosslineJumpLatch,
      crosslineJumpTarget,
      Mux(bpu.io.out.valid, pnpc, snpc)
    )
  )
  val npcIsSeq = Mux(
    io.redirect.valid,
    ...
    Mux(
      crosslineJumpLatch,
      false.B,
      Mux(crosslineJump, true.B, Mux(bpu.io.out.valid, false.B, true.B))
    )
  )
  Debug(
    "[NPC] %x %x %x %x %x %x\n",
