...
import core.uarch.branch.BTBType

object RV32_ShiftInstr extends HasInstrType {
...
object RV64_ShiftInstr extends HasInstrType {
...
object RV32I_ALUInstr extends HasInstrType {
  ...
  def SLLI64 = BitPat("b000000?_?????_?????_001_?????_00100_11")
...
object RV32I_BRUInstr extends HasInstrType {
  ...
  val table = Array(
    ...
    JALR -> List(InstrI, FuType.bru, BRUCtrl.jalr),
    BEQ -> List(InstrB, FuType.bru, BRUCtrl.beq),
    BNE -> List(InstrB, FuType.bru, BRUCtrl.bne),
    BLT -> List(InstrB, FuType.bru, BRUCtrl.blt),
    BGE -> List(InstrB, FuType.bru, BRUCtrl.bge),
    BLTU -> List(InstrB, FuType.bru, BRUCtrl.bltu),
    BGEU -> List(InstrB, FuType.bru, BRUCtrl.bgeu)
  )
  ...
  val bruCtrl2BtbTypeTable = List(
    ...
    BRUCtrl.call -> BTBType.J,
    BRUCtrl.ret -> BTBType.R,
...
object RVIInstr extends HasMarCoreParameter {
  val table =
    RV32I_ALUInstr.table ++ RV32I_BRUInstr.table ++ RV32I_LSUInstr.table ++
      (if (XLEN == 64) RV64I_Instr.table ++ RV64_ShiftInstr.table
       else RV32_ShiftInstr.table)
}
