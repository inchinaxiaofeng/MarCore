...
* `[6]`: Invert bit. 描述逻辑翻转位. 当这一位拉高时, 表示对不拉高的逻辑的翻转.
* `[5]`: Word bit. `XLEN \=\= 64 時, 當這一位拉高時，指 Word 數據類型. XLEN \=\= 32時, 這一位被拋棄.
* `[4]`: Unsigned bit. 無符號標誌位. 當這一位拉高時, 對 可拓展原數據 進行0拓展, 否則將會使用符號拓展
  * 对于 Encoded type 为 0 的类型而言, 代表u类型
  * 对于 Encoded type 为 1 的类型而言, 代表逻辑移动
* `[2,0]`: Encoded type bit. 編碼後類型, 用於在無法通過其他位分別時, 區分不同指令. 當編碼全爲 0 時用於區分不需要進行匹配

### BRU FuCtrl

* `[6]` Control transfer inst type. 轉移指令類型. `b0` Jump, `b1` Branch
* `[5]` Func call and ret. 函數調用返回類. 拉高代表當前跳轉被識別爲函數調用與返回. 這一位如果被指定1, 將會與RAS發生交互.
* `[4]` Indirect bit. 非直接目標標誌位. 拉高代表跳轉目標要基於Reg計算, 爲0代表跳轉目標是固定的(基於立即數或立即數與PC的計算)
* `[3]` Invert bit. 方向翻轉位. 當這位爲1時, 轉移方向翻轉.
* `[2]` Reserve
* `[1:0]` Encoded type bit. 編碼後類型, 用於在不能通過 1Hot 信號區分時, 標誌不同指令.
