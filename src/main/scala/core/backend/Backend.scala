...
import core.backend.unit._
import top.Settings
import blackbox._

import core.uarch.interfaces.{DecodeIO, CommitIO}
...
class Backend_inorder(implicit val p: MarCoreConfig) extends MarCoreModule {
  ...
  Debug("------------------------ Backend ------------------------\n")
  Debug(
    "flush = %b, {{{ ==%x%x>[isu]<%x---%x>[exu]<%x---%x>[wbu] }}}\n",
    io.flush.asUInt,
    isu.io.in(0).ready,
    isu.io.in(1).ready,
    isu.io.out.valid,
  ...
  Debug(
    isu.io.out.valid,
    "ISU: pc = 0x%x instr = 0x%x pnpc = 0x%x\n",
    isu.io.out.bits.cf.pc,
    isu.io.out.bits.cf.instr,
    isu.io.out.bits.cf.pnpc
  )
  Debug(
    exu.io.in.valid,
    "EXU: pc = 0x%x instr = 0x%x pnpc = 0x%x\n",
    exu.io.in.bits.cf.pc,
    exu.io.in.bits.cf.instr,
    exu.io.in.bits.cf.pnpc
  )
  Debug(
    wbu.io.in.valid,
    "WBU: pc = 0x%x instr = 0x%x pnpc = 0x%x\n",
    wbu.io.in.bits.decode.cf.pc,
    wbu.io.in.bits.decode.cf.instr,
