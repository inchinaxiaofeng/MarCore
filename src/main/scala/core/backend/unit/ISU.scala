...
import core.uarch.interfaces.{DecodeIO, WriteBackIO, ForwardIO}
...
class ISU(implicit val p: MarCoreConfig) extends MarCoreModule {
  ...
  val io = IO(new Bundle {
    val in = Vec(
      2,
      Flipped(Decoupled(new DecodeIO))
    ) // make in-order backend compatible with high performance frontend
  ...
  val dontForward =
    (io.forward.fuType =/= FuType.alu) && (io.forward.fuType =/= FuType.lsu)
  ...
  io.out.bits.data.srcA := Mux1H(
    List(
      (io.in(0).bits.ctrl.srcAType === SrcType.pc) -> SignExt(
        io.in(0).bits.cf.pc,
        AddrBits
      ),
      ...
      ((io
        ...
        .bits
        .ctrl
        .srcAType =/= SrcType.pc) && !srcAForwardNextCycle && !srcAForward) -> rf
        .read(rfSrcA)
    )
  )
  io.out.bits.data.srcB := Mux1H(
    List(
      ...
      ((io
        ...
        .bits
        .ctrl
        .srcBType === SrcType.reg) && !srcBForwardNextCycle && !srcBForward) -> rf
        .read(rfSrcB)
    )
  )
  ...
  val wbClearMask = Mux(
    io.wb.rfWen && !isDepend(io.wb.rfDest, io.forward.wb.rfDest, forwardRfWen),
    sb.mask(io.wb.rfDest),
    0.U(NRReg.W)
  )
  ...
  if (Settings.get("EnableDifftest") && Settings.get("DiffTestGPR")) {
    for (i <- 0 until 32) io.gpr.get.regs(i) := rf.read(i.U)
  }

  Debug(
    io.out.fire,
    "[ISSUE] pc %x npc %x instr %x FuType(alu lsu mdu csr mou) %d srcA %x srcB %x imm %x\n",
    io.out.bits.cf.pc,
    io.out.bits.cf.pnpc,
    io.out.bits.cf.instr,
    io.out.bits.ctrl.fuType,
    io.out.bits.data.srcA,
    io.out.bits.data.srcB,
    io.out.bits.data.imm
  )
//	Debug(io.forward.valid, "[Forward] RfWen %d RfDest %d RfData %x FuType %d(alu lsu mdu csr mou)\n", io.forward.wb.rfWen, io.forward.wb.rfDest, io.forward.wb.rfData, io.forward.fuType)
}
