...
// 	}
//
// 	if (Settings.get("TraceLSU")) Debug(io.out.fire, "[LSU-AGU] state %x inv %x inr %x\n", state, io.in.valid, io.in.ready)
//
// 	// Controled by FSM
...
// 	}
//
// 	if (Settings.get("TraceLSU")) Debug(dmem.req.fire, "[LSU] addr %x, size %x, wdata_raw %x, isStore %x\n", addr, ctrl(1, 0), io.wdata, isStore)
//
// 	val size = ctrl(1, 0)
...
//
// 	io.out.valid := Mux(/*state =/= s_idle || */io.ioLoadAddrMisaligned || io.ioStoreAddrMisaligned, true.B, Mux(partialLoad, state === s_partialLoad, dmem.resp.fire && (state === s_wait_resp)))
// 	io.in.ready := state === s_idle
//
// 	if (Settings.get("TraceLSU")) Debug(io.out.fire, "[LSU-EXECUNIT] state %x Resp %x lm %x sm %x\n", state, dmem.resp.fire, io.ioLoadAddrMisaligned, io.ioStoreAddrMisaligned)
//
...
// 	))
//
// 	if (Settings.get("TraceLSU")) Debug(dmem.req.ready && dmem.resp.ready, "[LSU] state %x Write %x ReqAddr %x Fire %x Data %x AddrLatch %x RDataLatch %x RDataPartialLoad %x\n", state, dmem.req.bits.write, dmem.req.bits.addr, dmem.req.fire, dmem.req.bits.data, addrLatch(2, 0), rdataLatch, rdataPartialLoad)
//
...
//
// 	if (Settings.get("TraceLSU")) Debug(io.ioLoadAddrMisaligned || io.ioStoreAddrMisaligned, "[EXCEPTION] misaligned addr detected\n")
// }
