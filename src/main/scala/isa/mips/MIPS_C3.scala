...
import defs._
import module.fu.mips.ALUCtrl
import module.fu.LSUCtrl
import module.fu.mips.MDUCtrl
import module.fu.BTBtype

object MIPS32_ArithInstr extends HasMips32_InstrType {
...
// FIXME
object MIPS32_BJInstr extends HasMips32_InstrType { // Need Different BRU
    ...
    val table = Array (
        BEQ     -> List(InstrI, FuType.bru, ALUCtrl.beq),
        BNE     -> List(InstrI, FuType.bru, ALUCtrl.bne),
        BLEZ    -> List(InstrI, FuType.bru, ALUCtrl.blez),
        BLTZ    -> List(InstrI, FuType.bru, ALUCtrl.bltz),
        BGEZ    -> List(InstrI, FuType.bru, ALUCtrl.bgez),
        BGTZ    -> List(InstrI, FuType.bru, ALUCtrl.bgtz),
        // I
        // I+1: PC = Cat(PC(GPRLEN-1, 28),index, 0.U(2.W))
        J       -> List(InstrJ, FuType.bru, ALUCtrl.j),
        // I: $31 = PC + 8
        // I+1: PC = Cat(PC(GPRLEN-1, 28),index, 0.U(2.W))
        JAL     -> List(InstrJ, FuType.bru, ALUCtrl.jal),
        // I: tmp = $rs, $rd = PC + 8
        // I+1: PC = tmp
        JALR    -> List(InstrR, FuType.bru, ALUCtrl.jalr),
        // I: tmp = $rs
        // I+1: PC = tmp
        JR      -> List(InstrR, FuType.bru, ALUCtrl.jr)
    )

    val bruCtrl2BtbTypeTable = List (
        ALUCtrl.beq     -> BTBtype.B,
        ALUCtrl.bne     -> BTBtype.B,
        ALUCtrl.blez    -> BTBtype.B,
        ALUCtrl.bgez    -> BTBtype.B,
        ALUCtrl.bgtz    -> BTBtype.B,
        ALUCtrl.bltz    -> BTBtype.B,
        ALUCtrl.j       -> BTBtype.J,
        ALUCtrl.jal     -> BTBtype.J,
        ALUCtrl.jalr    -> BTBtype.J,
        ALUCtrl.jr      -> BTBtype.I
    )
}

object MIPS32_LSInstr extends HasMips32_InstrType {
...
object MIPS32_ShiftInstr extends HasMips32_InstrType {
...
object MIPS32_LogicInstr extends HasMips32_InstrType {
...
object MIPSInstrC3 extends HasMarCoreParameter {
    val table = MIPS32_ArithInstr.table ++ MIPS32_LogicInstr.table ++ MIPS32_ShiftInstr.table ++
