...
class Decoder(implicit val p: MarCoreConfig) extends MarCoreModule with HasInstrType {
	...
	val decodeList = ListLookup(instr, InstructionsMIPS.DecodeDefault, InstructionsMIPS.DecodeTable)
...
//		)}
	val instrType :: fuType :: fuCtrl :: Nil = 
		InstructionsMIPS.DecodeDefault.zip(decodeList).map { case (instr, dec) => dec }
	io.out.bits := DontCare

	io.out.bits.ctrl.fuType := fuType
	io.out.bits.ctrl.fuCtrl := fuCtrl

	val isDelaySlot = RegEnable(Mux(ALUCtrl.isBru(instr), true.B, false.B), false.B, io.in.valid)
	...
	when (fuType === FuType.bru) {
		...
		when (isLink(rfDest) && fuCtrl === ALUCtrl.jalr) { io.out.bits.ctrl.fuCtrl := ALUCtrl.call }
	...
	io.isBranch := VecInit(MIPS32_BJInstr.table.map(i => i._2.tail(1) === fuCtrl).toIndexedSeq).asUInt.orR &&
		fuType === FuType.bru
}
