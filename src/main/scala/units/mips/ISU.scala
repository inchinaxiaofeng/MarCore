...
class ISU(implicit val p: MarCoreConfig) extends MarCoreModule with HasRegFileParameter {
	implicit val moduleName: String = this.name
		val io = IO(new Bundle {
			val in = Vec(2, Flipped(Decoupled(new DecodeIO_MIPS))) // make in-order backend compatible with high performance frontend
	...
	val wbClearMask = Mux(io.wb.rfWen && !isDepend(io.wb.rfDest, io.forward.wb.rfDest, forwardRfWen),
		sb.mask(io.wb.rfDest), 0.U(NRReg.W))
...
//	Debug(io.out.fire, "[ISSUE] pc %x npc %x instr %x FuType(alu lsu mdu csr mou) %d srcA %x srcB %x imm %x\n", io.out.bits.cf.pc, io.out.bits.cf.pnpc, io.out.bits.cf.instr, io.out.bits.ctrl.fuType, io.out.bits.data.srcA, io.out.bits.data.srcB, io.out.bits.data.imm)
