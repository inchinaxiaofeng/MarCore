...
You can download it [scala-sbt.org](https://www.scala-sbt.org/download.html).
`mill` is another Scala/Java build tool without obscure DSL like `SBT`.
You can download it [github-mill](https://github.com/com-lihaoyi/mill/releases)

### `Verilator`

The test with `svsim` needs `Verilator` installed.
See `Verilator` installation instructions [verilator.org](https://verilator.org/guide/latest/install.html).

### `Coursier` (recommend)

We recommend install `Coursier` to Install Scala and it's language server.

### `npm` (recommend)

We need `node.js/npm` environment to install `bashls`,
because `mason` wanna install Scala environment by `npm`.

### `markdownlint-cli2` (recommend)

`markdownlint-cli2` is a faster wrapper lint than `markdownlint-cli`.

### `homebrew` (recommend)

Based on our test, `homebrew`, or `linuxbrew`, is the easiest way to download package.

## How to get started

### Pick an Editor

We recommend [`LazyVim`](https://www.lazyvim.org/) as your Editor.
...
Follow the installation step in [verilator.org](https://verilator.org/guide/latest/install.html).
...
#### Install Scala(Chisel) language server (with `LazyVim`)
...
* Install Java 8 Or Java 11/17 Or newer

* Install `npm`.

* Toggle `lang.scala`, `dap.core`(`nvim-dap`) in `LazyExtras`.

* Then Re open `LazyVim`
...
建议使用Nvim进行开发控制。在Mason中下载scala相关控件，
并且基于<https://get-coursier.io/docs/cli-install> 下载cs，通过cs下载相关更多控件即可。
...
1. `Fork` 和 `Clone` 仓库
首先， `Fork` [中心仓库](TODO)到自己的 `GitHub` 账号下，并 `Clone` 到本地环境。
在后续开发中，基于开发分支进行协作，所有的 `Pull Request(PR)` 和合并操作都将在该分支上进行。
